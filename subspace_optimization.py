...
class orth_subspaces(object):
    ...
    def _cost(self, Y): 
        ...
        dim_new = np.vstack((0,np.expand_dims(self.dim,1)))
        f=[]
        for j in range(self.numSubspaces):
            Qj = Y[:,int(np.sum(dim_new[0:j+1])):int(np.sum(dim_new[0:j+1])+dim_new[j+1])]
            Cj = self.covM[j]
            normFactj = self.normFact[j] 
            f = np.hstack((f,normFactj*np.trace(np.dot(np.dot(Qj.T,Cj),Qj)))) # core piece: normalize projected variance
        print(f)
        f = 1-np.sum(f)
...
#       return -gradQ
    ...
    def estimate_orth_subspaces(self, DataStruct):
        ...
        # initialize solver
        solver = TrustRegions(mingradnorm=1e-8,minstepsize=1e-16,logverbosity=1)
